...
- [ ] **Step 1: `backend/vitest.config.mts` 작성**
...
- [ ] **Step 2: 기존 `backend/vitest.config.ts` 삭제**

```bash
rm backend/vitest.config.ts
```

- [ ] **Step 3: `backend/package.json` 스크립트 수정**
...
- [ ] **Step 4: 기존 unit 테스트가 여전히 통과하는지 확인**

Run: `cd backend && pnpm test`
Expected: 기존 `*.spec.ts` 전부 PASS (동작 변화 없이 실행 방식만 바뀜 확인).

- [ ] **Step 5: integration 프로젝트가 빈 상태로 정상 인식되는지 확인**
...
- [ ] **Step 2: `EnviromentUtil.ts`가 `DB_HOST`/`DB_PORT`/`REDIS_HOST`/`REDIS_PORT` 환경변수를 YAML 값보다 우선하도록 수정**

기존 `getEnvFrom`은 YAML만 읽는다. `global-setup.ts`가 심어줄 testcontainer 포트를 받을 수 있도록, 있으면 덮어쓰게 한다(없으면 기존과 100% 동일하게 동작 — 로컬/프로덕션 영향 없음).
...
- [ ] **Step 3: `backend/test/setup/global-setup.ts` 작성**
...
export default async function setup(): Promise<void> {
	...
	const postgresContainer = await new PostgreSqlContainer('postgres:18.4')
		...
		.withExposedPorts({ container: 5432, host: env.database.port })
		.withReuse()
		.start();
	...
	const redisContainer = await new RedisContainer('redis:7.4')
		...
		.withExposedPorts({ container: 6379, host: env.redis.port })
		.withReuse()
		.start();
...
`env.database.port`/`env.redis.port`는 `env/env.test.yml`의 5433/6380을 그대로 고정 호스트 포트로 재사용 — `docker-compose.test.yml`과 충돌하면(이미 떠 있으면) 포트가 이미 점유돼 있을 수 있으니, `pnpm test:int` 실행 전엔 `pnpm test:e2e:down` 상태여야 한다(둘 다 같은 포트를 씀 — 별도 포트로 분리하지 않은 이유는 이번 범위에서 두 스택을 동시에 띄울 필요가 없기 때문).

- [ ] **Step 4: 컨테이너가 실제로 뜨는지 확인**
...
- [ ] **Step 1: `backend/test/setup/workerContext.ts` 작성**
...
- [ ] **Step 2: `backend/test/setup/worker-setup.ts` 작성**
...
- [ ] **Step 1: 실패(아직 존재하지 않는 동작 확인)하는 스파이크 테스트 작성**
...
- Produces: `ServiceIntTestHelper.of(imports): Promise<ServiceIntTestHelper>`, `.getService<T>(service: Type<T>): T`, `.startTransaction(): Promise<void>`, `.rollbackTransaction(): Promise<void>`, `.moduleClose(): Promise<void>` — Task 7의 `auth.service.int-spec.ts`가 이 4개 메서드만 사용한다.

- [ ] **Step 1: `createTestingModule.ts`에 MikroORM 옵션 오버라이드 파라미터 추가**
...
- [ ] **Step 2: `ServiceIntTestHelper.ts` 작성**
...
- [ ] **Step 3: 헬퍼 자체를 검증하는 테스트 작성(Task 7의 축소판, 헬퍼 전용)**
...
describe('ServiceIntTestHelper', () => {
	...
	it('트랜잭션 롤백 후에는 저장한 엔티티가 남지 않는다', async () => {
		...
		// when
		await helper.rollbackTransaction();
		const freshEm = helper.getService(EntityManager);
		const found = await freshEm.findOne(User, { githubId: 'helper-github-id' });
...
- Create: `backend/libs/common/test-helper/factory/UserFactory.int-spec.ts`
...
export abstract class BaseFactory<T extends object> {
	...
	async saveMany(count: number, overrides?: Partial<T>): Promise<T[]> {
		...
		await this.em.persistAndFlush(instances);
		return instances;
	}
...
- [ ] **Step 4: `backend/libs/common/test-helper/factory/UserFactory.int-spec.ts` 작성**
...
describe('UserFactory', () => {
	...
	it('override 없이 저장하면 기본값으로 채워진 유저가 저장된다', async () => {
		// given & when
		const user = await factoryManager.userFactory.save();
	...
	it('githubId를 override하면 그 값으로 저장된다', async () => {
		// given & when
		const user = await factoryManager.userFactory.save({ githubId: 'override-github-id' });
...
git commit -m "test: BaseFactory/FactoryManager/UserFactory 픽스처 추가"
```
...
- [ ] **Step 2: 실행해서 실패 확인(작성 전 상태 재현용 — 이미 구현된 서비스라 사실상 통과해야 정상)**
...
- [ ] **Step 3: (구현은 이미 존재하므로 생략) — 필요 시 `AuthService`는 수정하지 않는다**
...
- [ ] **Step 5: 다중 worker 병렬 실행에서도 격리가 깨지지 않는지 확인**
...
- [ ] `pnpm test`(unit), `pnpm test:int`(integration) 로컬 통과
- [ ] `pnpm test:int` 다중 worker 병렬 실행 시 데이터 격리 유지(Task 7 Step 5)
- [ ] 같은 프로세스 반복 실행에도 이전 데이터 안 남음(Task 5/6/7의 각 `rollbackTransaction` 테스트로 확인됨)
- [ ] MikroORM 트랜잭션 컨텍스트 공유 스파이크 통과(Task 4)
- [ ] 파일럿 서비스(Auth) integration 테스트 1개 이상, Factory 픽스처 기반(Task 7)
